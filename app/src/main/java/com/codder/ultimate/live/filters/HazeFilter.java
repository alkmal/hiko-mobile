...
public class HazeFilter extends BaseFilter implements TwoParameterFilter {
    ...
    private static final String TAG = "HazeFilter";
    ...
    public void onCreate(int programHandle) {
        ...
        try {
            ...
            if (mDistanceLocation < 0) {
                Log.e(TAG, "Uniform location for 'distance' is invalid.");
            }
            if (mSlopeLocation < 0) {
                Log.e(TAG, "Uniform location for 'slope' is invalid.");
            }
        } catch (Exception e) {
            mDistanceLocation = -1;
            mSlopeLocation = -1;
    ...
    protected void onPreDraw(long timestampUs, @NonNull float[] transformMatrix) {
        ...
        try {
            if (mDistanceLocation >= 0) {
                ...
                Egloo.checkGlError("glUniform1f(distance)");
            } else {
                Log.w(TAG, "mDistanceLocation is invalid, uniform not set.");
            }
            if (mSlopeLocation >= 0) {
                GLES20.glUniform1f(mSlopeLocation, mSlope);
                Egloo.checkGlError("glUniform1f(slope)");
            } else {
                Log.w(TAG, "mSlopeLocation is invalid, uniform not set.");
            }
        } catch (Exception e) {
            Log.e(TAG, "Error in onPreDraw: " + e.getMessage(), e);
        }
    }
}
